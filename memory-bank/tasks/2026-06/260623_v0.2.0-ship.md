...
- ✅ **GitHub release v0.2.0** — 9 assets (both Mac DMGs + updater tarballs, Linux deb/rpm/AppImage,
  Windows x64/arm64). https://github.com/rubezhanin/agency-agents-app/releases/tag/v0.2.0
- ✅ **Auto-update LIVE** at `agency-agents-app.rubezhanin.app/updater.json` — covers `darwin-aarch64` **and** `darwin-x86_64`.
...
- Endpoint `https://agency-agents-app.rubezhanin.app/updater.json`, served by **Caddy on `umacbookpro`** from `~/Sites/agency-agents/`
  (the `agency-agents-app.rubezhanin.app` docroot — a sibling vhost to the live `brew-browser.zerologic.com/updater.json`).
