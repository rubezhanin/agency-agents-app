...
/// Hermes plugin renderer. Unlike the rest of the supported tools, a Hermes
/// install is a **directory** under `~/.hermes/plugins/agency-agents-router/`,
/// not a single rendered file. The renderer in `render::hermes` produces the
/// directory; the install path lives in `commands::hermes`. This split is
/// intentional — `render::render` below is one-file-only and is the wrong
/// abstraction for directory producers.
pub mod hermes;
